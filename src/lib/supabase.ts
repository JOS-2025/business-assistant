...
export function handleSupabaseError(error: unknown, operationType: OperationType, path: string | null) {
  ...
  // Only log to console, don't throw if it's a common network error that we handle gracefully elsewhere
  console.error('Supabase Error Context:', JSON.stringify(errInfo));
  
  const errorToThrow = new Error(errorMessage);
  (errorToThrow as any).isNetworkError = errorMessage.includes("Connection lost") || errorMessage.includes("not configured");
