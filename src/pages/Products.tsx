...
import { Plus, Search, Package, Trash2, Edit2, AlertCircle, Clock, Building2 } from "lucide-react";
...
export default function Products() {
    ...
    const fetchData = async () => {
      try {
        ...
        try {
          ...
          if (prodData.length > 0 || branchData.length > 0) {
  ...
  const filteredProducts = products.filter(p => {
    ...
    const matchesStock = stockFilter === "All" || 
      (stockFilter === "Low Stock" && p.stock <= (p.min_stock_level || 5)) || 
      (stockFilter === "In Stock" && p.stock > (p.min_stock_level || 5));
    ...
    return matchesSearch && matchesCategory && matchesBranch && matchesStock && matchesExpiry;
  });
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
            {branches.length > 0 && (
              <div className="flex flex-col gap-1 min-w-[120px]">
                <label className="text-[10px] uppercase font-bold text-slate-400 px-1">Branch</label>
                <select
