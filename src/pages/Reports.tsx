...
export default function Reports() {
  ...
  const dailyDataRaw = eachDayOfInterval({
    ...
    const grossSales = dayTrans.filter(t => t.type === 'sale').reduce((sum, t) => sum + t.amount, 0);
    const taxes = dayTrans.filter(t => t.type === 'sale').reduce((sum, t) => sum + (t.tax_amount || 0), 0);
    const sales = grossSales - taxes;
  ...
  const totalGrossSales = transactions.filter(t => t.type === 'sale').reduce((sum, t) => sum + t.amount, 0);
  const totalTaxes = transactions.filter(t => t.type === 'sale').reduce((sum, t) => sum + (t.tax_amount || 0), 0);
  const totalSales = totalGrossSales - totalTaxes;
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
            ...
            <p className="text-xl font-bold text-slate-900">KES {(totalSales || 0).toLocaleString()}</p>
          </div>
          <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
            ...
            <p className="text-xl font-bold text-slate-900">KES {(totalExpenses || 0).toLocaleString()}</p>
          </div>
          <div className="col-span-2 bg-emerald-600 p-4 rounded-2xl shadow-lg shadow-emerald-600/20 text-white">
            <div className="flex justify-between items-start mb-2">
              <div>
                ...
                <p className="text-2xl font-bold">KES {(netProfit || 0).toLocaleString()}</p>
              </div>
